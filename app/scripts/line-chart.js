...
directive('linechart', [
  'n3utils', '$window', '$timeout', function(n3utils, $window, $timeout) {
    ...
    link = function(scope, element, attrs, ctrl) {
      ...
      /*  This part is added to original.
          Allows chart updates without a complete redraw. We use it for highlighting
          the selected X value.
      */
      scope.updateWithoutRedraw = function() {
...
mod.factory('n3utils', [
  '$window', '$log', '$rootScope', function($window, $log, $rootScope) {
    return {
      ...
      /*  This part has been modified from the original. We render not only a circle,
          but a group with a circle and a line and dot that we'll use to highlight the
          max value.
      */
      drawDots: function(svg, axes, data, options, handlers) {
        ...
        .attr('transform', function(d) { 
          return 'translate('+[axes.xScale(d.x), axes[d.axis + 'Scale'](d.y + d.y0)]+')';
        });
        dot.append('circle').attr({
          'class': 'dot',
          'r': function(d) {
            return d.dotSize;
          },
          'cx': function(d) {
            return 0;//axes.xScale(d.x);
          },
          'cy': function(d) {
            return 0;//axes[d.axis + 'Scale'](d.y + d.y0);
          }
        }).style({
          'stroke': 'white',
          'stroke-width': '2px'
        ...
        dot.append('line').attr({
          ...
          'y1': 0,
          'y2': function(d) {
            return axes[d.axis + 'Scale'](0) - axes[d.axis + 'Scale'](d.y + d.y0);
          }
        });
      ...
      /*  This part is added to the original. The function finds the largest y
          and adds a class 'max', which can be used for highlighting.
      */
      ...
      annotateSelectedX: function(svg, xValue) {
        var _s = svg.selectAll('.dot-group')
